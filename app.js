...
function renderHome() {
  ...
  return `
    ...
    <section class="section">
      ...
      <div class="card-list">
        ${state.loading ? skeletons(2) : activeNeeds.length ? activeNeeds.map(renderNeedCard).join('') : emptyState('🤝', 'Відкритих потреб немає', 'Усі опубліковані запити вже закрито.')}
      </div>
    </section>
...
function renderNeeds() {
  ...
  return `
    ...
    <div class="card-list">
      ${state.loading ? skeletons(3) : items.length ? items.map(renderNeedCard).join('') : emptyState('🔎', 'Нічого не знайдено', 'Спробуй змінити категорію або місто.')}
    </div>
  `;
}
...
function renderPoints() {
  ...
  return `
    ...
    <div class="card-list">
      ${state.loading ? skeletons(3) : points.length ? points.map(renderPointCard).join('') : emptyState('📍', 'Точок не знайдено', 'Зміни фільтр або повідом координаторам про нову точку.')}
    </div>
    <section class="section">
    ...
    <section class="section">
      <div class="form-card">
        ...
        <div class="form-grid">
          ...
          <button class="btn btn-soft btn-wide" data-action="open-admin">Відкрити редактор даних</button>
...
function filterPoints(items) {
  ...
  return [...items]
    .map((item) => state.coords && item.lat && item.lng ? { ...item, distance: haversine(state.coords.lat, state.coords.lng, item.lat, item.lng) } : item)
...
function handleAction(action, target) {
  const handlers = {
    ...
    'open-admin': () => window.open('./admin.html', '_blank', 'noopener'),
...
function openMap(id) {
  const point = state.points.find((item) => item.id === id);
  if (!point) return;
  const url = point.mapUrl || (point.lat && point.lng ? `https://www.google.com/maps/search/?api=1&query=${point.lat},${point.lng}` : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${point.address}, ${point.city}`)}`);
  openExternal(url);
}
...
async function copyAndShare(text) {
  try { await navigator.clipboard.writeText(text); showToast('Текст скопійовано. Надішли його координаторам.'); } catch {}
  const coordinator = CONFIG.coordinatorTelegram;
  if (coordinator && coordinator !== 'https://t.me/') openExternal(coordinator);
  else openExternal(`https://t.me/share/url?url=&text=${encodeURIComponent(text)}`);
}
...
function demoBanner() {
  ...
  return `<div class="demo-banner"><div class="demo-icon">!</div><div><strong>Демонстраційний режим</strong><span>Наведені потреби й точки є прикладом. Перед публічним запуском заміни файли у папці <code>data</code> та вимкни <code>demoMode</code> у <code>config.js</code>.</span></div></div>`;
}
...
function openExternal(url) { if (!url) return; tg?.openLink ? tg.openLink(url) : window.open(url, '_blank', 'noopener'); }
...
function formatDate(value) { if (!value) return '—'; return new Intl.DateTimeFormat('uk-UA', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' }).format(new Date(value)); }
